...
#define NRF24_RATE				1 // 0 - 250kbps, 1 - 1Mbps, 2 - 2Mbps
...
//	NRF24_CMD_W_REGISTER | NRF24_REG_RF_CH,		NRF24_RF_CHANNEL, // RF channel
	...
	NRF24_CMD_W_REGISTER | NRF24_REG_SETUP_RETR,(0b0000<<NRF24_BIT_ARD) | (0b1111<<NRF24_BIT_ARC), // Auto Retransmit Delay, Re-Transmit on fail
	NRF24_CMD_W_REGISTER | NRF24_REG_RF_SETUP,	0b111 | (NRF24_RATE == 0 ? (1<<NRF24_BIT_RF_DR_LOW)|(0<<NRF24_BIT_RF_DR_HIGH) : NRF24_RATE == 2 ? (0<<NRF24_BIT_RF_DR_LOW)|(1<<NRF24_BIT_RF_DR_HIGH) : 0), // Data rate: Max power (0b111)
	NRF24_CMD_W_REGISTER | NRF24_REG_EN_AA,		0b000001, // Enable ‘Auto Acknowledgment’ for pipes
	NRF24_CMD_W_REGISTER | NRF24_REG_EN_RXADDR,	0b000001, // Enable data pipes
...
xReadPayload:		
	...
	} else if((NRF24_ReadRegister(NRF24_REG_FIFO_STATUS) & (1<<NRF24_BIT_RX_EMPTY)) == 0) goto xReadPayload;
	return pipe;
}
...
uint8_t NRF24_Transmit(uint8_t *payload) // Transmit payload, return 0 if success, 1 - max retransmit count reached, 2 - module not response.
{
	...
	do {
		...
		if(st & (1<<NRF24_BIT_TX_DS)) return 0;
		if(st & (1<<NRF24_BIT_MAX_RT)) { // stop if sent or max retransmit reached
